...
def pregunta_10():
    ...
    """
    x = tbl0.copy()
    x = x.groupby('_c1').agg({'_c2': lambda var: sorted(list(var))})
    for ind, fil in x.iterrows():
...
def pregunta_11():
    ...
    """
    x = tbl1.copy()
    x = x.groupby('_c0').agg({'_c4': lambda var: sorted(list(var))})
    for ind, fil in x.iterrows():
        fil['_c4'] = ",".join([str(num) for num in fil['_c4']])
    x.insert(0, '_c0', range(0, 40))
    return x
...
def pregunta_12():
    ...
    """
    x1 = tbl2.copy()
    x1['_c5'] = x1['_c5a'] + ':' + x1['_c5b'].astype(str)
    x = x1.groupby('_c0').agg({'_c5': lambda var: sorted(var)})
    for ind, fil in x.iterrows():
